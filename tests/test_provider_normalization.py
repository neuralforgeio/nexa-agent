...
from nexa.provider import LLMProvider  # instantiation check only
...
def test_active_provider_base_url_is_normalized(tmp_path, monkeypatch):
    ...
    import importlib
    importlib.reload(__import__("nexa.config", fromlist=["*"]))
    cfg = reg.get_active()
...
def test_llmprovider_from_active_uses_registry(tmp_path, monkeypatch):
    monkeypatch.setenv("NEXA_PROVIDER", "tokenrouter")
    import importlib, os
    import nexa.provider as p
    importlib.reload(p)
