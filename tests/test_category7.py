...
def test_homebrew_formula_parseable():
    from pathlib import Path
    text = Path("packaging/homebrew/openforge.rb").read_text(encoding="utf-8")
    assert "class Openforge < Formula" in text and "license" in text


def test_inno_setup_script_sane():
    from pathlib import Path
    text = Path("packaging/windows/inno/openforge.iss").read_text(encoding="utf-8")
...
def test_vscode_manifest_valid_json():
    ...
    assert data["name"] == "openforge" and "commands" not in data or "contributes" in data
