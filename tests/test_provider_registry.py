...
with secrets stored in ~/.nexa/secrets/providers.json.

Verifies:
    - NEXA_SECRETS_DIR is ~/.nexa/secrets/ with mode 0o600 (best effort).
...
from nexa.config import NEXA_HOME, NEXA_SECRETS_DIR
...
class TestSecretsDir:
    """Tests for the NEXA_SECRETS_DIR constant."""

    def test_secrets_dir_under_nexa_home(self) -> None:
        """NEXA_SECRETS_DIR must be ~/.nexa/secrets/."""
        assert NEXA_SECRETS_DIR == NEXA_HOME / "secrets"

    def test_secrets_dir_created_on_import(self) -> None:
        """NEXA_SECRETS_DIR exists after importing config."""
        assert NEXA_SECRETS_DIR.exists()
        assert NEXA_SECRETS_DIR.is_dir()
...
class TestProviderRegistryTest:
    ...
    async def test_test_returns_bool(self, tmp_path: Path, monkeypatch) -> None:
        ...
        with patch("nexa.provider_registry.check_provider_health", side_effect=fake_check):
