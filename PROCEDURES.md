...
git grep "ghp_" . ':!CONTINUATION_PROMPT.md' ':!FORGE_MASTER_PLAN.md' ':!worklog.md'
# Expect: nothing.
...
1. **Bump version files (single source of truth)**
   ...
   - [ ] `forge_web/package.json` → `"version": "X.Y.0"`
...
5. **Frontend headers**
   - [ ] `forge_web/app/page.tsx` EmptyState default `appVersion`.
   - [ ] `forge_web/components/SandboxPanel.tsx`, `WorkingProcess.tsx`, `SettingsPanel.tsx`,
...
7. **Branch / tag / release**
   - [ ] Demo branch `forge-demo-<8-hex>` exists locally and has been pushed.
   - [ ] `git checkout main`
   - [ ] `git merge --no-ff forge-demo-<8-hex> -m "..."`
   - [ ] `git tag -a vX.Y.0 -m "OpenForge vX.Y.0 — <headline>"`
...
- [ ] `pyproject.toml` and `forge_web/package.json` bumped.
...
- [ ] `git status` clean (no stray ``*.bak``, `*.tmp`, `*.openforge.tmp`).
- [ ] Dead exports removed from `forge_web/lib/theme.ts` (or marked with `# reserved for v5`).
- [ ] Unused components removed from `forge_web/components/`.
