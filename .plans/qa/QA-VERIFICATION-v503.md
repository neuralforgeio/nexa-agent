...
| P0-5 | ws endpoints missing `: WebSocket` annotation -> live 403 | PARTIAL-CONFIRMED (STATIC) | src/server.py:1029 `async def ws_terminal(websocket) -> None:` and :1244 `async def ws_approval(websocket):` both lack `: WebSocket` annotation. Live 403 not reproduced (OS). Static=CONFIRMED annotation absent; live-403=NME. |
| P0-6 | TUI chat panel never renders | CONFIRMED (STILL-BROKEN) | ui_tui/render/layout.py imports render_chat_area(35) but the only `.update()` calls: header(66), persona(79), work(80), calls(81), footer(88/91/94). No `layout["chat"].update(render_chat_area(state))`. |
| P0-7 | nexa/ shim missing (mandate) | CONFIRMED (STILL-BROKEN) | Test-Path nexa -> False. `python -c "import nexa"` -> ModuleNotFoundError. Stale `nexa_agent-4.2.2.dist-info` present in venv site-packages. |
...
| P1-7 | skills_panel ERROR not imported -> NameError | CONFIRMED (STILL-BROKEN) | skills_panel.py:22 imports {ACCENT,BORDER,MUTED,SUCCESS,TEXT,WARNING}; uses `ERROR` at :46 and :94. No ERROR import -> NameError on disabled-skill render. |
| P1-8 | chat_stream raises instead of yielding error tuple | CONFIRMED (STILL-BROKEN) | provider.py:224 `client = await self._get_client()` is OUTSIDE `try:` (which starts :230). `_get_client` raise -> raises, not yields ('error',...). |
| P1-9 | panels.py render_tool_log accesses persona.detail_open -> AttributeError | CONFIRMED (STILL-BROKEN) | panels.py:198 `if state.persona and state.persona.detail_open:`. Select-String repo-wide: `detail_open` ONLY at panels.py:198, never defined. |
| P1-10 | /exit & /quit advertised but not in _DISPATCH | CONFIRMED (STILL-BROKEN) | commands.py:72-73 advertise /exit,/quit. `_DISPATCH` (393-412) lacks both. dispatch():428 `_DISPATCH.get(cmd)->None` -> returns "". |
| P1-11 | /api/sandbox/build timeout 180 vs terminal MAX_TIMEOUT 60 | CONFIRMED (STILL-BROKEN) | src/server.py:1616 `timeout=180.0`; tools/terminal_tool.py:74 `MAX_TIMEOUT = 60.0`. run_terminal_command clamps/raises >60 -> mismatch. |
| P1-12 | /ws/approval lacks verify_token_ws | CONFIRMED (STILL-BROKEN) | ws_approval (:1244-1251) has NO verify_token_ws call; ws_terminal (:1055) DOES. |
