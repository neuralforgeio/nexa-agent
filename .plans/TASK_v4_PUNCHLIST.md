# OpenForge v4.0.0 — Punch List (What Was Reported vs What Was Fixed)
...
1. `SandboxPanel`'s dev-server autodetect listed `http://localhost:3000` —
   which is the Forge UI *itself*. So the sandbox iframe loaded Forge inside
   Forge, and the iframe-then-parent paint raced. (This is the "sandbox
...
- `SandboxPanel.tsx` — `detectPreview()` now **skips forge's own port** and
...
### 2. Sandbox iframe showed the Forge UI itself
...
preview any file from `FORGE_WORKSPACE` directly.
...
- The empty-state greeting has been elevated to z.ai-style centered
  typography with the Forge rhombus logo.
...
contains a `tools` array the model build can't handle. Forge now:
...
(`FORGE E2E OK`, `Hello` → `Halo! 👋 Ada yang bisa saya bantu hari ini?`).
...
- **`create_tool`** — the agent can extend itself by writing new tools
  into `~/.openforge/tools/` (loaded next turn).
...
- **User-scaffolded content**: `~/.openforge/USER.md`, `~/.openforge/PROCEDURES.md`
  are injected into every system prompt — personalization without editing code.
- **Global CLI install** — `pip install -e .` puts `forge` / `forge-chat`
